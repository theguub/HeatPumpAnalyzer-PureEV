...
def calculate_lmtd(delta_t1, delta_t2):
    ...
    """
    if delta_t1 <= 0 or delta_t2 <= 0:  # Invalid temperature differences
        print(f"Invalid temperature differences: ΔT1 = {delta_t1}, ΔT2 = {delta_t2}")
        return None
...
# Common parameters
T_ambient_C = -10  # Outside temperature (°C)
...
for refrigerant in refrigerants:
    ...
    T_evap_C = T_ambient_C - T_evap_buffer  # Evaporator temperature (K) (buffer below ambient)
    T_cond_C = T_cabin_C + T_cond_buffer  # Condenser temperature (K) (buffer above cabin)
    ...
    # 2. Compressor Outlet (Superheated vapor, non-isentropic compression)
    ...
    # 4. Expansion Valve Outlet (Isenthalpic process)
    ...
    COP_heating = Q_H / W_compressor

    # Maximum COP (Carnot COP)
    COP_max = T_cabin_R / (T_cabin_R - T_ambient_R)
    ...
    P_max = heating_capacity_cond / COP_max  # Maximum power consumption (W)
    P_actual = heating_capacity_cond / COP_heating  # Actual power consumption (W)

    # Compare with required heat transfer
    c_p_air = 1005  # J/kg·K
    air_mass_flow_rate = 0.1  # kg/s
    required_heat_transfer = air_mass_flow_rate * c_p_air * (T_cabin_C - T_ambient_C)  # (W or J/s)

    # Assumed heat transfer coefficients (U values)
    U_condenser = 500  # W/m²·K
    U_evaporator = 300  # W/m²·K
   ...
   # Condenser
    ...
    A_condenser = cooling_capacity_evap / (U_condenser * LMTD_condenser)  # Condenser area (m²)
    ...
    E_drive_cold = 20  # Cold weather
    P_heating_cold = heating_capacity_cond / 1000  # kW for heating

    # Conversion factor for distance
    km_to_miles = 0.621371

    # Calculate total energy consumption rates (kWh/100 km)
    E_total_cold = E_drive_cold + (P_heating_cold / 100)  # Cold weather

    # Calculate ranges (km)
    battery_capacity_kWh = 100
    range_cold_km = (battery_capacity_kWh / E_total_cold) * 100
 
    # Convert ranges to miles
    range_cold_miles = range_cold_km * km_to_miles


    # Append results
    cops.append(COP_heating)
    heating_capacities.append(heating_capacity_cond / 1e3)  # Convert to kW
    ...
    print("\n=== Coefficient of Performance (COP) ===")
    print(f"Actual COP (Heating): {round(COP_heating, 2)}")
    print(f"Maximum COP (Carnot): {round(COP_max, 2)}")
    # Verify if the actual COP is realistic and efficient
    if 3.5 <= COP_heating <= 6 and COP_heating < COP_max:
        print(f"The actual COP ({round(COP_heating, 2)}) is within the realistic and efficient range (3.5 to 6) and is less than the maximum COP ({round(COP_max, 2)}).")
    elif COP_heating >= COP_max:
        print(f"The actual COP ({round(COP_heating, 2)}) exceeds the maximum COP ({round(COP_max, 2)}), which is unrealistic.")
    else:
        print(f"The actual COP ({round(COP_heating, 2)}) is outside the realistic and efficient range (3.5 to 6) or invalid.")


    print("\n=== Heating and Power Calculations ===")
    print(f"Heat Delivered (Q_H): {round(Q_H / 1e3, 2)} kJ/kg")
    print(f"Heating Capacity: {round(heating_capacity_cond / 1e3, 2)} kW")
    print(f"Required Heat Transfer: {round(required_heat_transfer / 1e3, 2)} kW")
    print(f"Capacity Check: {'Sufficient' if heating_capacity_cond >= required_heat_transfer else 'Insufficient'}")
    ...
    print(f"Energy Consumption (Cold, kWh/100 km): {round(E_total_cold, 2)}") 
    print(f"Range (Cold, km): {round(range_cold_km, 2)} km")   
    print(f"Range (Cold, miles): {round(range_cold_miles, 2)} miles")
    ...
    if 250 <= range_cold_miles <= 500:
        print(f"Cold Weather Range ({round(range_cold_miles, 2)} miles): Within the average range of 250 to 500 miles for Pure EVs.")
    else:
        print(f"Cold Weather Range ({round(range_cold_miles, 2)} miles): Outside the average range of 250 to 500 miles for Pure EVs.")
...
plt.title("COP Comparison for Refrigerants at Cold Temps.", fontsize=14)
...
plt.title("Heating Capacity Comparison for Refrigerants at Cold Temps.", fontsize=14)
plt.xlabel("Refrigerants", fontsize=12)
